...
export function filterData(
  ...
  const filtered = data.filter((record) => {
    ...
    // 4. Business type filter - only apply when explicit B2B/B2C business type filtering is active
    // This is only relevant when the segment dimension has separate b2b/b2c hierarchies
    // (not when B2B/B2C are just regular segment names in the hierarchy)
    // Skip this filter entirely - B2B/B2C are treated as regular hierarchical segments
